"""
Kaggle Solution: Embeddings for High-Cardinality Features
========================================
Demonstrates embeddings for high-cardinality features with comprehensive analysis and visualization.
"""
...
def apply_entity_embedding_encoding(df, categorical_cols=None):
    """Apply entity_embedding encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'entity_embedding' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'entity_embedding' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_entity_embedding'] = le.fit_transform(df[col].astype(str))
            elif 'entity_embedding' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_entity_embedding'] = df[col].map(target_mean)
            elif 'entity_embedding' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_entity_embedding'] = df[col].map(counts)
            elif 'entity_embedding' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_autoencoder_encoding(df, categorical_cols=None):
    """Apply autoencoder encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'autoencoder' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'autoencoder' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_autoencoder'] = le.fit_transform(df[col].astype(str))
            elif 'autoencoder' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_autoencoder'] = df[col].map(target_mean)
            elif 'autoencoder' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_autoencoder'] = df[col].map(counts)
            elif 'autoencoder' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_pca_reduced_encoding(df, categorical_cols=None):
    """Apply pca_reduced encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'pca_reduced' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'pca_reduced' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_pca_reduced'] = le.fit_transform(df[col].astype(str))
            elif 'pca_reduced' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_pca_reduced'] = df[col].map(target_mean)
            elif 'pca_reduced' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_pca_reduced'] = df[col].map(counts)
            elif 'pca_reduced' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/19_embedding_features/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['entity_embedding', 'autoencoder', 'pca_reduced']
