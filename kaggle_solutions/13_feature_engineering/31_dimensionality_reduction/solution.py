"""
Kaggle Solution: Dimensionality Reduction
========================
Demonstrates dimensionality reduction with comprehensive analysis and visualization.
"""
...
def apply_pca_encoding(df, categorical_cols=None):
    """Apply pca encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'pca' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'pca' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_pca'] = le.fit_transform(df[col].astype(str))
            elif 'pca' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_pca'] = df[col].map(target_mean)
            elif 'pca' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_pca'] = df[col].map(counts)
            elif 'pca' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_kernel_pca_encoding(df, categorical_cols=None):
    """Apply kernel_pca encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'kernel_pca' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'kernel_pca' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_kernel_pca'] = le.fit_transform(df[col].astype(str))
            elif 'kernel_pca' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_kernel_pca'] = df[col].map(target_mean)
            elif 'kernel_pca' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_kernel_pca'] = df[col].map(counts)
            elif 'kernel_pca' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_tsne_encoding(df, categorical_cols=None):
    """Apply tsne encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'tsne' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'tsne' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_tsne'] = le.fit_transform(df[col].astype(str))
            elif 'tsne' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_tsne'] = df[col].map(target_mean)
            elif 'tsne' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_tsne'] = df[col].map(counts)
            elif 'tsne' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_umap_encoding(df, categorical_cols=None):
    """Apply umap encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'umap' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'umap' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_umap'] = le.fit_transform(df[col].astype(str))
            elif 'umap' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_umap'] = df[col].map(target_mean)
            elif 'umap' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_umap'] = df[col].map(counts)
            elif 'umap' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_truncated_svd_encoding(df, categorical_cols=None):
    """Apply truncated_svd encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'truncated_svd' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'truncated_svd' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_truncated_svd'] = le.fit_transform(df[col].astype(str))
            elif 'truncated_svd' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_truncated_svd'] = df[col].map(target_mean)
            elif 'truncated_svd' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_truncated_svd'] = df[col].map(counts)
            elif 'truncated_svd' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/31_dimensionality_reduction/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    print("=" * 90)
    print("31. Dimensionality Reduction")
    print("=" * 90)
    ...
    methods = ['pca', 'kernel_pca', 'tsne', 'umap', 'truncated_svd']
