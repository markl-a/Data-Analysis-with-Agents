...
====================================
Demonstrates feature selection methods comparison with comprehensive analysis and visualization.
"""
...
def apply_rfe_encoding(df, categorical_cols=None):
    """Apply rfe encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'rfe' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'rfe' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_rfe'] = le.fit_transform(df[col].astype(str))
            elif 'rfe' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_rfe'] = df[col].map(target_mean)
            elif 'rfe' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_rfe'] = df[col].map(counts)
            elif 'rfe' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_selectkbest_encoding(df, categorical_cols=None):
    """Apply selectkbest encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'selectkbest' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'selectkbest' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_selectkbest'] = le.fit_transform(df[col].astype(str))
            elif 'selectkbest' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_selectkbest'] = df[col].map(target_mean)
            elif 'selectkbest' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_selectkbest'] = df[col].map(counts)
            elif 'selectkbest' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_lasso_encoding(df, categorical_cols=None):
    """Apply lasso encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'lasso' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'lasso' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_lasso'] = le.fit_transform(df[col].astype(str))
            elif 'lasso' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_lasso'] = df[col].map(target_mean)
            elif 'lasso' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_lasso'] = df[col].map(counts)
            elif 'lasso' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_tree_based_encoding(df, categorical_cols=None):
    """Apply tree_based encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'tree_based' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'tree_based' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_tree_based'] = le.fit_transform(df[col].astype(str))
            elif 'tree_based' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_tree_based'] = df[col].map(target_mean)
            elif 'tree_based' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_tree_based'] = df[col].map(counts)
            elif 'tree_based' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_mutual_info_encoding(df, categorical_cols=None):
    """Apply mutual_info encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'mutual_info' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'mutual_info' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_mutual_info'] = le.fit_transform(df[col].astype(str))
            elif 'mutual_info' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_mutual_info'] = df[col].map(target_mean)
            elif 'mutual_info' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_mutual_info'] = df[col].map(counts)
            elif 'mutual_info' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/30_feature_selection_methods/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    print("=" * 90)
    print("30. Feature Selection Methods")
    print("=" * 90)
    ...
    methods = ['rfe', 'selectkbest', 'lasso', 'tree_based', 'mutual_info']
