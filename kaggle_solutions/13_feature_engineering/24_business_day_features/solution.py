"""
Kaggle Solution: Business Day and Holiday Features
=================================
Demonstrates business day and holiday features with comprehensive analysis and visualization.
"""
...
def apply_business_day_encoding(df, categorical_cols=None):
    """Apply business_day encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'business_day' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'business_day' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_business_day'] = le.fit_transform(df[col].astype(str))
            elif 'business_day' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_business_day'] = df[col].map(target_mean)
            elif 'business_day' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_business_day'] = df[col].map(counts)
            elif 'business_day' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_holiday_encoding(df, categorical_cols=None):
    """Apply holiday encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'holiday' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'holiday' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_holiday'] = le.fit_transform(df[col].astype(str))
            elif 'holiday' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_holiday'] = df[col].map(target_mean)
            elif 'holiday' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_holiday'] = df[col].map(counts)
            elif 'holiday' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_workday_encoding(df, categorical_cols=None):
    """Apply workday encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'workday' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'workday' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_workday'] = le.fit_transform(df[col].astype(str))
            elif 'workday' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_workday'] = df[col].map(target_mean)
            elif 'workday' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_workday'] = df[col].map(counts)
            elif 'workday' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_quarter_encoding(df, categorical_cols=None):
    """Apply quarter encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'quarter' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'quarter' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_quarter'] = le.fit_transform(df[col].astype(str))
            elif 'quarter' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_quarter'] = df[col].map(target_mean)
            elif 'quarter' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_quarter'] = df[col].map(counts)
            elif 'quarter' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/24_business_day_features/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    print("=" * 90)
    print("24. Business Day Features")
    print("=" * 90)
    ...
    methods = ['business_day', 'holiday', 'workday', 'quarter']
