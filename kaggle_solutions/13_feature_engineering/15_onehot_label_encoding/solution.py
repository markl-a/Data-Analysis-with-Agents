"""
Kaggle Solution: One-Hot Encoding vs Label Encoding
==================================
Demonstrates one-hot encoding vs label encoding with comprehensive analysis and visualization.
"""
...
def apply_one_hot_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'one_hot' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'one_hot' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_one_hot'] = le.fit_transform(df[col].astype(str))
            elif 'one_hot' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_one_hot'] = df[col].map(target_mean)
            elif 'one_hot' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_one_hot'] = df[col].map(counts)
            elif 'one_hot' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_label_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'label' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'label' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_label'] = le.fit_transform(df[col].astype(str))
            elif 'label' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_label'] = df[col].map(target_mean)
            elif 'label' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_label'] = df[col].map(counts)
            elif 'label' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_ordinal_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'ordinal' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'ordinal' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_ordinal'] = le.fit_transform(df[col].astype(str))
            elif 'ordinal' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_ordinal'] = df[col].map(target_mean)
            elif 'ordinal' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_ordinal'] = df[col].map(counts)
            elif 'ordinal' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_binary_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'binary' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'binary' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_binary'] = le.fit_transform(df[col].astype(str))
            elif 'binary' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_binary'] = df[col].map(target_mean)
            elif 'binary' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_binary'] = df[col].map(counts)
            elif 'binary' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/15_onehot_label_encoding/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    print("=" * 90)
    print("15. Onehot Label Encoding")
    print("=" * 90)
    ...
    methods = ['one_hot', 'label', 'ordinal', 'binary']
