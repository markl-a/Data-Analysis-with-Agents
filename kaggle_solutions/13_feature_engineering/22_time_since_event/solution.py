"""
Kaggle Solution: Time Since Event Features
=========================
Demonstrates time since event features with comprehensive analysis and visualization.
"""
...
def apply_days_since_encoding(df, categorical_cols=None):
    """Apply days_since encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'days_since' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'days_since' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_days_since'] = le.fit_transform(df[col].astype(str))
            elif 'days_since' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_days_since'] = df[col].map(target_mean)
            elif 'days_since' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_days_since'] = df[col].map(counts)
            elif 'days_since' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_weeks_since_encoding(df, categorical_cols=None):
    """Apply weeks_since encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'weeks_since' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'weeks_since' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_weeks_since'] = le.fit_transform(df[col].astype(str))
            elif 'weeks_since' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_weeks_since'] = df[col].map(target_mean)
            elif 'weeks_since' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_weeks_since'] = df[col].map(counts)
            elif 'weeks_since' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_months_since_encoding(df, categorical_cols=None):
    """Apply months_since encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'months_since' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'months_since' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_months_since'] = le.fit_transform(df[col].astype(str))
            elif 'months_since' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_months_since'] = df[col].map(target_mean)
            elif 'months_since' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_months_since'] = df[col].map(counts)
            elif 'months_since' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/22_time_since_event/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['days_since', 'weeks_since', 'months_since']
