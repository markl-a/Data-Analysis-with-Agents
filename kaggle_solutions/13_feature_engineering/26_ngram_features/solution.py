"""
Kaggle Solution: N-Gram Features
===============
Demonstrates n-gram features with comprehensive analysis and visualization.
"""
...
def apply_unigram_encoding(df, categorical_cols=None):
    """Apply unigram encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'unigram' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'unigram' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_unigram'] = le.fit_transform(df[col].astype(str))
            elif 'unigram' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_unigram'] = df[col].map(target_mean)
            elif 'unigram' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_unigram'] = df[col].map(counts)
            elif 'unigram' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_bigram_encoding(df, categorical_cols=None):
    """Apply bigram encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'bigram' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'bigram' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_bigram'] = le.fit_transform(df[col].astype(str))
            elif 'bigram' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_bigram'] = df[col].map(target_mean)
            elif 'bigram' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_bigram'] = df[col].map(counts)
            elif 'bigram' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_trigram_encoding(df, categorical_cols=None):
    """Apply trigram encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'trigram' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'trigram' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_trigram'] = le.fit_transform(df[col].astype(str))
            elif 'trigram' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_trigram'] = df[col].map(target_mean)
            elif 'trigram' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_trigram'] = df[col].map(counts)
            elif 'trigram' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_char_ngram_encoding(df, categorical_cols=None):
    """Apply char_ngram encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'char_ngram' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'char_ngram' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_char_ngram'] = le.fit_transform(df[col].astype(str))
            elif 'char_ngram' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_char_ngram'] = df[col].map(target_mean)
            elif 'char_ngram' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_char_ngram'] = df[col].map(counts)
            elif 'char_ngram' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/26_ngram_features/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['unigram', 'bigram', 'trigram', 'char_ngram']
