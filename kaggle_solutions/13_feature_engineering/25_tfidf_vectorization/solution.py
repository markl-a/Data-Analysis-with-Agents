"""
Kaggle Solution: TF-IDF and Count Vectorization
==============================
Demonstrates tf-idf and count vectorization with comprehensive analysis and visualization.
"""
...
def apply_tfidf_encoding(df, categorical_cols=None):
    """Apply tfidf encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'tfidf' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'tfidf' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_tfidf'] = le.fit_transform(df[col].astype(str))
            elif 'tfidf' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_tfidf'] = df[col].map(target_mean)
            elif 'tfidf' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_tfidf'] = df[col].map(counts)
            elif 'tfidf' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_count_vectorizer_encoding(df, categorical_cols=None):
    """Apply count_vectorizer encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'count_vectorizer' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'count_vectorizer' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_count_vectorizer'] = le.fit_transform(df[col].astype(str))
            elif 'count_vectorizer' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_count_vectorizer'] = df[col].map(target_mean)
            elif 'count_vectorizer' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_count_vectorizer'] = df[col].map(counts)
            elif 'count_vectorizer' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_l2_norm_encoding(df, categorical_cols=None):
    """Apply l2_norm encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'l2_norm' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'l2_norm' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_l2_norm'] = le.fit_transform(df[col].astype(str))
            elif 'l2_norm' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_l2_norm'] = df[col].map(target_mean)
            elif 'l2_norm' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_l2_norm'] = df[col].map(counts)
            elif 'l2_norm' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/25_tfidf_vectorization/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['tfidf', 'count_vectorizer', 'binary', 'l2_norm']
