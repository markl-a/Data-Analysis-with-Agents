"""
Kaggle Solution: Target Encoding and Mean Encoding
=================================
Demonstrates target encoding and mean encoding with comprehensive analysis and visualization.
"""
...
def generate_categorical_data(n_samples=3000):
    """Generate synthetic data with categorical features."""
    categories_low = ['A', 'B', 'C', 'D']
    categories_med = [f'Cat_{i}' for i in range(20)]
    categories_high = [f'Item_{i}' for i in range(100)]
    
    cat_low = np.random.choice(categories_low, n_samples, p=[0.4, 0.3, 0.2, 0.1])
    cat_med = np.random.choice(categories_med, n_samples)
    cat_high = np.random.choice(categories_high, n_samples)
    
    num_feat1 = np.random.normal(100, 20, n_samples)
    num_feat2 = np.random.exponential(50, n_samples)
    
    # Target with categorical influence
    cat_low_effect = {'A': 50, 'B': 30, 'C': 20, 'D': 10}
    target = (
        np.array([cat_low_effect[c] for c in cat_low]) +
        0.5 * num_feat1 +
        0.3 * num_feat2 +
...
def apply_target_mean_encoding(df, categorical_cols=None):
    """Apply target_mean encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'target_mean' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'target_mean' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_target_mean'] = le.fit_transform(df[col].astype(str))
            elif 'target_mean' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_target_mean'] = df[col].map(target_mean)
            elif 'target_mean' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_target_mean'] = df[col].map(counts)
            elif 'target_mean' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_smoothed_target_encoding(df, categorical_cols=None):
    """Apply smoothed_target encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'smoothed_target' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'smoothed_target' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_smoothed_target'] = le.fit_transform(df[col].astype(str))
            elif 'smoothed_target' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_smoothed_target'] = df[col].map(target_mean)
            elif 'smoothed_target' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_smoothed_target'] = df[col].map(counts)
            elif 'smoothed_target' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_loo_encoding(df, categorical_cols=None):
    """Apply loo encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'loo' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'loo' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_loo'] = le.fit_transform(df[col].astype(str))
            elif 'loo' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_loo'] = df[col].map(target_mean)
            elif 'loo' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_loo'] = df[col].map(counts)
            elif 'loo' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_weight_of_evidence_encoding(df, categorical_cols=None):
    """Apply weight_of_evidence encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'weight_of_evidence' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'weight_of_evidence' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_weight_of_evidence'] = le.fit_transform(df[col].astype(str))
            elif 'weight_of_evidence' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_weight_of_evidence'] = df[col].map(target_mean)
            elif 'weight_of_evidence' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_weight_of_evidence'] = df[col].map(counts)
            elif 'weight_of_evidence' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/16_target_encoding/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['target_mean', 'smoothed_target', 'loo', 'weight_of_evidence']
