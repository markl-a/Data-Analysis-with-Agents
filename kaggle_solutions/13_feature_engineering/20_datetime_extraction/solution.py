"""
Kaggle Solution: Date/Time Feature Extraction
============================
Demonstrates date/time feature extraction with comprehensive analysis and visualization.
"""
...
def generate_temporal_data(n_samples=3000):
    ...
    target = (
        100 +
        20 * np.sin(2 * np.pi * month / 12) +  # Yearly seasonality
        10 * np.sin(2 * np.pi * hour / 24) +  # Daily seasonality
        5 * (dayofweek >= 5) +  # Weekend effect
        0.01 * np.arange(n_samples) +  # Trend
        np.random.normal(0, 5, n_samples)
    )
    
    df = pd.DataFrame({
        'datetime': dates,
        'value': target + np.random.normal(0, 3, n_samples),
        'target': target
    })
...
def apply_hour_encoding(df, categorical_cols=None):
    """Apply hour encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'hour' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'hour' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_hour'] = le.fit_transform(df[col].astype(str))
            elif 'hour' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_hour'] = df[col].map(target_mean)
            elif 'hour' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_hour'] = df[col].map(counts)
            elif 'hour' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_day_encoding(df, categorical_cols=None):
    """Apply day encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'day' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'day' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_day'] = le.fit_transform(df[col].astype(str))
            elif 'day' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_day'] = df[col].map(target_mean)
            elif 'day' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_day'] = df[col].map(counts)
            elif 'day' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_month_encoding(df, categorical_cols=None):
    """Apply month encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'month' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'month' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_month'] = le.fit_transform(df[col].astype(str))
            elif 'month' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_month'] = df[col].map(target_mean)
            elif 'month' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_month'] = df[col].map(counts)
            elif 'month' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_season_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'season' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'season' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_season'] = le.fit_transform(df[col].astype(str))
            elif 'season' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_season'] = df[col].map(target_mean)
            elif 'season' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_season'] = df[col].map(counts)
            elif 'season' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_weekend_encoding(df, categorical_cols=None):
    """Apply weekend encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'weekend' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'weekend' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_weekend'] = le.fit_transform(df[col].astype(str))
            elif 'weekend' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_weekend'] = df[col].map(target_mean)
            elif 'weekend' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_weekend'] = df[col].map(counts)
            elif 'weekend' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/20_datetime_extraction/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['hour', 'day', 'month', 'season', 'weekend']
