"""
Kaggle Solution: Text Length and Complexity Features
===================================
Demonstrates text length and complexity features with comprehensive analysis and visualization.
"""
...
def generate_text_data(n_samples=2000):
    ...
    neutral_words = ['product', 'item', 'thing', 'object', 'service', 'experience']
    ...
    for i in range(n_samples):
        ...
        texts.append(' '.join(words) + ' ' + ' '.join(np.random.choice(neutral_words, 3)))
...
def apply_length_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'length' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'length' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_length'] = le.fit_transform(df[col].astype(str))
            elif 'length' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_length'] = df[col].map(target_mean)
            elif 'length' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_length'] = df[col].map(counts)
            elif 'length' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_word_count_encoding(df, categorical_cols=None):
    """Apply word_count encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'word_count' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'word_count' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_word_count'] = le.fit_transform(df[col].astype(str))
            elif 'word_count' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_word_count'] = df[col].map(target_mean)
            elif 'word_count' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_word_count'] = df[col].map(counts)
            elif 'word_count' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_avg_word_length_encoding(df, categorical_cols=None):
    """Apply avg_word_length encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'avg_word_length' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'avg_word_length' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_avg_word_length'] = le.fit_transform(df[col].astype(str))
            elif 'avg_word_length' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_avg_word_length'] = df[col].map(target_mean)
            elif 'avg_word_length' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_avg_word_length'] = df[col].map(counts)
            elif 'avg_word_length' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_unique_words_encoding(df, categorical_cols=None):
    """Apply unique_words encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'unique_words' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'unique_words' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_unique_words'] = le.fit_transform(df[col].astype(str))
            elif 'unique_words' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_unique_words'] = df[col].map(target_mean)
            elif 'unique_words' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_unique_words'] = df[col].map(counts)
            elif 'unique_words' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/27_text_complexity/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['length', 'word_count', 'avg_word_length', 'unique_words']
