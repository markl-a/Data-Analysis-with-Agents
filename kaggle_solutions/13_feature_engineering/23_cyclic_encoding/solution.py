"""
Kaggle Solution: Cyclic Feature Encoding
=======================
Demonstrates cyclic feature encoding with comprehensive analysis and visualization.
"""
...
def apply_sin_cos_encoding(df, categorical_cols=None):
    """Apply sin_cos encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'sin_cos' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'sin_cos' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_sin_cos'] = le.fit_transform(df[col].astype(str))
            elif 'sin_cos' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_sin_cos'] = df[col].map(target_mean)
            elif 'sin_cos' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_sin_cos'] = df[col].map(counts)
            elif 'sin_cos' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_fourier_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'fourier' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'fourier' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_fourier'] = le.fit_transform(df[col].astype(str))
            elif 'fourier' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_fourier'] = df[col].map(target_mean)
            elif 'fourier' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_fourier'] = df[col].map(counts)
            elif 'fourier' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_periodic_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'periodic' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'periodic' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_periodic'] = le.fit_transform(df[col].astype(str))
            elif 'periodic' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_periodic'] = df[col].map(target_mean)
            elif 'periodic' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_periodic'] = df[col].map(counts)
            elif 'periodic' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/23_cyclic_encoding/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['sin_cos', 'fourier', 'periodic']
