"""
Kaggle Solution: Frequency Encoding
==================
Demonstrates frequency encoding with comprehensive analysis and visualization.
"""
...
def apply_count_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'count' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'count' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_count'] = le.fit_transform(df[col].astype(str))
            elif 'count' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_count'] = df[col].map(target_mean)
            elif 'count' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_count'] = df[col].map(counts)
            elif 'count' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_frequency_encoding(df, categorical_cols=None):
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'frequency' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'frequency' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_frequency'] = le.fit_transform(df[col].astype(str))
            elif 'frequency' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_frequency'] = df[col].map(target_mean)
            elif 'frequency' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_frequency'] = df[col].map(counts)
            elif 'frequency' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_log_frequency_encoding(df, categorical_cols=None):
    """Apply log_frequency encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'log_frequency' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'log_frequency' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_log_frequency'] = le.fit_transform(df[col].astype(str))
            elif 'log_frequency' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_log_frequency'] = df[col].map(target_mean)
            elif 'log_frequency' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_log_frequency'] = df[col].map(counts)
            elif 'log_frequency' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_rank_encoding(df, categorical_cols=None):
    """Apply rank encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'rank' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'rank' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_rank'] = le.fit_transform(df[col].astype(str))
            elif 'rank' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_rank'] = df[col].map(target_mean)
            elif 'rank' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_rank'] = df[col].map(counts)
            elif 'rank' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/17_frequency_encoding/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['count', 'frequency', 'log_frequency', 'rank']
