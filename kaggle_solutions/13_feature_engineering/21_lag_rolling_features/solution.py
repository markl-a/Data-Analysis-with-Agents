"""
Kaggle Solution: Lag Features and Rolling Window Statistics
==========================================
Demonstrates lag features and rolling window statistics with comprehensive analysis and visualization.
"""
...
def apply_lag_encoding(df, categorical_cols=None):
    """Apply lag encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            ...
            if 'lag' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'lag' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_lag'] = le.fit_transform(df[col].astype(str))
            elif 'lag' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_lag'] = df[col].map(target_mean)
            elif 'lag' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_lag'] = df[col].map(counts)
            elif 'lag' in ['sin_cos', 'fourier']:
                # Cyclic encoding
...
def apply_rolling_mean_encoding(df, categorical_cols=None):
    """Apply rolling_mean encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'rolling_mean' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'rolling_mean' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_rolling_mean'] = le.fit_transform(df[col].astype(str))
            elif 'rolling_mean' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_rolling_mean'] = df[col].map(target_mean)
            elif 'rolling_mean' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_rolling_mean'] = df[col].map(counts)
            elif 'rolling_mean' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_rolling_std_encoding(df, categorical_cols=None):
    """Apply rolling_std encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'rolling_std' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'rolling_std' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_rolling_std'] = le.fit_transform(df[col].astype(str))
            elif 'rolling_std' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_rolling_std'] = df[col].map(target_mean)
            elif 'rolling_std' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_rolling_std'] = df[col].map(counts)
            elif 'rolling_std' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def apply_ewm_encoding(df, categorical_cols=None):
    """Apply ewm encoding."""
    df_encoded = df.copy()
    ...
    for col in categorical_cols:
        if col in df.columns:
            # Method-specific encoding logic
            if 'ewm' in ['one_hot', 'binary']:
                encoded = pd.get_dummies(df[col], prefix=col)
                df_encoded = pd.concat([df_encoded.drop(col, axis=1), encoded], axis=1)
            elif 'ewm' in ['label', 'ordinal']:
                le = LabelEncoder()
                df_encoded[f'{col}_ewm'] = le.fit_transform(df[col].astype(str))
            elif 'ewm' in ['target_mean', 'smoothed_target']:
                # Target encoding (simple mean)
                if 'target' in df.columns:
                    target_mean = df.groupby(col)['target'].mean()
                    df_encoded[f'{col}_ewm'] = df[col].map(target_mean)
            elif 'ewm' in ['count', 'frequency']:
                counts = df[col].value_counts()
                df_encoded[f'{col}_ewm'] = df[col].map(counts)
            elif 'ewm' in ['sin_cos', 'fourier']:
                # Cyclic encoding
                unique_vals = df[col].nunique()
                df_encoded[f'{col}_sin'] = np.sin(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
                df_encoded[f'{col}_cos'] = np.cos(2 * np.pi * df[col].astype('category').cat.codes / unique_vals)
...
def plot_comprehensive_analysis(df, results_dict, feature_importance=None):
    ...
    plt.savefig('/home/user/Data-Analysis-with-Chatbots/kaggle_solutions/13_feature_engineering/21_lag_rolling_features/analysis.png', dpi=300, bbox_inches='tight')
    print("   Plot saved!")
    plt.show()
...
def main():
    ...
    methods = ['lag', 'rolling_mean', 'rolling_std', 'ewm']
