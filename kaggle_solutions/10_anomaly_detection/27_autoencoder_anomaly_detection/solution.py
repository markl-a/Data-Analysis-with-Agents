...
def main():
    ...
    plt.savefig('27_autoencoder_anomaly_detection_roc_pr_curves.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Score distributions
    fig = plot_score_distributions(y_test, scores_dict)
    plt.savefig('27_autoencoder_anomaly_detection_score_distributions.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2D visualization
    fig = plot_2d_visualization(X_test, y_test, y_pred_dict, "Autoencoder for Anomaly Detection")
    plt.savefig('27_autoencoder_anomaly_detection_2d_visualization.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    results_df.to_csv('27_autoencoder_anomaly_detection_results.csv', index=False)
    print("\nResults saved!")
    print("\n" + "="*80)
