...
DENCLUE Clustering
==================
Comprehensive implementation of denclue clustering with multiple variants,
...
def main():
    ...
    for ds_name, (X, y_true, true_k) in datasets.items():
        ...
        for method_name, result in results.items():
            ...
            plt.savefig(f'/tmp/22_denclue_clustering_{ds_name}_{method_name}.png',
        ...
        if len(results) > 1:
            ...
            plt.savefig(f'/tmp/22_denclue_clustering_{ds_name}_metrics.png',
                       dpi=300, bbox_inches='tight')
            plt.close()

            fig = plot_comparison_grid(X, results, title_prefix=f"{ds_name.capitalize()} - ")
            plt.savefig(f'/tmp/22_denclue_clustering_{ds_name}_grid.png',
                       dpi=300, bbox_inches='tight')
            plt.close()
